clc;
close all;

% STEP 2
filePath = 'E:\uni\6th term\Data transmission\project\p1\farzan.jpg';
img = imread(filePath);
%figure;
%imshow(img)

% ---------------
%figure;
%imhist(img)

% ---------------
grayImg = rgb2gray(img)
%figure;
%imshow(grayImg)
% imwrite(grayImg,'GrayImage.bmp');
% imwrite(grayImg,'GrayImage.tiff');

% ---------------
% totalEnergy = sum(grayImg(:));
totalEnergy = sum(grayImg(:).^2);
display(totalEnergy);
% alternative
F = fft2(grayImg);
magImage = abs(F).^2;
energy = sum(magImage(:));
display(energy);
%
ftshifted_normalized_F = fftshift(log(abs(F)));
% imshow(ftshifted_normalized_F, []);

% -----------------

noisedPicture = imnoise(grayImg,'gaussian');
%{ 
figure;
imshow(noisedPicture);
%}

% -----------------
ft = fftshift(log(abs(fft2(grayImg)))); % log is for reducing the large difference between max and data
% figure;
% imshow(ft, []);

% --------------------

%%% 1st method %%% https://www.geeksforgeeks.org/matlab-ideal-lowpass-filter-in-image-processing/

% Saving the size of the input_image in pixels-
% M : no of rows (height of the image)
% N : no of columns (width of the image)
[M, N] = size(noisedPicture);
  
% Getting Fourier Transform of the input_image
% using MATLAB library function fft2 (2D fast fourier transform)  
FT_img = fft2(double(noisedPicture));
  
% Assign Cut-off Frequency  
% D0 = 30; % one can change this value accordingly
D0 = 270; % 120 - 150 - 250 - 300 - 350

% Designing filter
u = 0:(M-1);
idx = find(u>M/2);
u(idx) = u(idx)-M;
v = 0:(N-1);
idy = find(v>N/2);
v(idy) = v(idy)-N;
  
% MATLAB library function meshgrid(v, u) returns
% 2D grid which contains the coordinates of vectors
% v and u. Matrix V with each row is a copy 
% of v, and matrix U with each column is a copy of u
[V, U] = meshgrid(v, u);
  
% Calculating Euclidean Distance
D = sqrt(U.^2+V.^2);
  
% Comparing with the cut-off frequency and 
% determining the filtering mask
H = double(D <= D0);
  
% Convolution between the Fourier Transformed
% image and the mask
G = H.*FT_img;
  
% Getting the resultant image by Inverse Fourier Transform
% of the convoluted image using MATLAB library function 
% ifft2 (2D inverse fast fourier transform)  
output_image = real(ifft2(double(G)));
  
% Displaying Input Image and Output Image
% subplot(2, 1, 1), imshow(noisedPicture),
% subplot(2, 1, 2), imshow(output_image, [ ]);
figure;
imshow(noisedPicture)
figure;
imshow(output_image, [ ])
imwrite(output_image, "1st.jpg")

% ---------------

%%% 2nd method %%% https://www.mathworks.com/help/images/ref/medfilt2.html
Kmedian = medfilt2(noisedPicture);
figure;
imshow(Kmedian);
imwrite(Kmedian, "2nd.jpg")


% -----------------
% SNR
x = im2double(grayImg);
y = im2double(abs(noisedPicture - grayImg));
signalNoiseRatio = snr(x,y);
display(signalNoiseRatio);
% -----------------

% PEAK SNR
[peaksnr, outputSNR] = psnr(noisedPicture , grayImg);  
fprintf('\n The Peak-SNR value is %0.4f', peaksnr );


